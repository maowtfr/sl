...
tabs = ["LAGA Bauschutt", "LAGA Boden","SR"]#, "LAGA Boden", "Gebäude-Wiki", "Geotechnik", "Hydrogeologie" ]
...
if active_tab == "LAGA Bauschutt":
    ...
    # Auswertung
        ...
        with st.beta_expander("Wichtige Hinweise"):
            ...
            if Z_Klassifikation==">Z2":
                hinweise=1
                st.error("**Kritische Überschreitung**: Die Zuordnungswerte eines oder mehrerer Analyseparameter überschreiten den oberen Grenzbereich der Z2-Klassifikation und liegen somit im Anwendungsbereich der Deponieverordnung (2009). Eine Plausibilitätskontrolle wird empfohlen.")
            if Z_Klassifikation=="Z2" or Z_Klassifikation==">Z2":
                hinweise=1
                st.warning("**Hohe Zuordnungswerte**: Die Zuordnungswerte einer oder mehrerer Analyseparameter sind mit einer Zuordnung zur Klassifikation von mindestens Z2 deutlich erhöht.")
            if Kw_A > 300:
                hinweise = 1
                st.warning('**Kohlenwasserstoffe**: Kohlenwasserstoffe liegen oberhalb des Wertes von 300 mg/kg. Gemäß LAGA 1997 Bauschutt gilt hierfür, dass Überschreitungen, die auf Asphaltanteile zurückzuführen sind, kein Ausschlusskriterium für eine Zuordnung zu den entsprechenden Klassifikationen darstellen.')
            if Pak16_A > 15:
                hinweise = 1
                st.warning(
    ...
    st.write("Dieser Abschnitt dient der Neu-Benennung von Dateinamen entsprechend der geforderten Vorgaben.")
    single_file=st.file_uploader("Datei wählen")
    single_file = __file__
    folder=os.path.dirname(single_file)
    dateien = glob.glob('*.pdf')

    for i in dateien:
        pdfReader = PyPDF2.PdfFileReader(i)
        pageObj = pdfReader.getPage(0)
        pageObk = pdfReader.getPage(1)
        pageObj_O = pageObj.extractText()
        pageOb_k = pageObk.extractText()
        if 'LISCON' in pageObj_O:
            auftraggeber_A_raw = pageObj_O.split('Prüfbericht')[1]
            Pn = auftraggeber_A_raw.split('LISCON')[0]
            Pn=Pn.lstrip()
            seite_A_raw = pageObj_O.split('Seite')[1]
            seite = seite_A_raw.split('von')[1]
            pages="("+str(seite.lstrip())+" Seiten)"
            pages=pages.lstrip()
            name=str(Pn)+" "+str(pages)
            #name=name.lstrip()
            if '\n' in name:
                name=name.replace('\n', '')
            #name=name.lstrip()
            #seite=seite.lstrip()      
            newfile=str(name)+'.pdf'
            fileObj=Path(newfile)
            if fileObj.is_file()==False:
                os.rename(i,newfile)
            print(name)

        if 'graner' in pageObj_O:
            auftraggeber_A_raw = pageObj_O.split('Prüfbericht')[1]
            Pn = auftraggeber_A_raw.split('Auftraggeber')[0]
            Pn=Pn.lstrip()
            seite_A_raw = pageObj_O.split('Seite')[1]
            seite = seite_A_raw.split('von')[1]
            seite=seite.lstrip()
